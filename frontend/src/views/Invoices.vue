  ...
  <div class="page">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <input v-model="searchText" type="text" placeholder="Search invoice..." class="search-input" />
      </div>

      <p v-if="error" class="error-msg">{{ error }}</p>
      <div v-else-if="loading" class="state">Loading invoice records...</div>
      <div v-else-if="!filteredInvoices().length" class="state">No invoice records found.</div>
      ...
      <div v-else>
        ...
        <div class="mobile-only mobile-cards">
          <div v-for="(inv, index) in filteredInvoices()" :key="inv.docNo" class="mobile-card">
...
.doc { font-weight: 600; color: #0f766e; }
.muted { color: #94a3b8; }
...
@media (max-width: 768px) {
  ...
  .mobile-card { background: white; border: 1px solid #e2e8f0; border-radius: 10px; padding: 16px; }
  .mobile-card-header { display: flex; justify-content: space-between; align-items: center; padding-bottom: 12px; border-bottom: 1px solid #f1f5f9; margin-bottom: 12px; }
  .mobile-doc { font-weight: 600; color: #0f766e; font-size: 15px; }
  .mobile-amount { font-weight: 700; color: #0f172a; font-size: 16px; }
  .mobile-card-body { display: flex; flex-direction: column; gap: 8px; }
  .mobile-row { display: flex; justify-content: space-between; font-size: 14px; }
